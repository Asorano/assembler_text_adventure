...
ifeq ($(CONFIG),debug)
    # Debug configuration
    ASMFLAGS = -f win64 -g -DDEBUG=1 -DDEBUG_LOG=1 -I$(INCLUDE_DIR)
    ...
    ASMFLAGS = -f win64 -g -DDEV_BUILD=1 -DDEBUG_LOG=1 -DSKIP_INTRO=1 -I$(INCLUDE_DIR)
    CFLAGS = -c -O1 -DDEV_BUILD=1 -DSKIP_INTRO=1 -I$(INCLUDE_DIR)
    LINKFLAGS = /debug /map:$(BIN_DIR)/$(TARGET)$(MAP_EXT) /pdb:$(BIN_DIR)/$(TARGET)$(PDB_EXT) $(COMMON_LINKFLAGS)
else
    # Release configuration
    ASMFLAGS = -f win64 -I$(INCLUDE_DIR)
